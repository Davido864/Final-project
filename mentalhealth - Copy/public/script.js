...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    let lastSelectedOption = null; // Keep track of the last selected option
    ...
    if (generateQuizButton) {
        ...
        generateQuizButton.addEventListener('click', function(e) {
            ...
            if (topic === 'topic1' && difficulty === 'option1' && numberOfQuestions === 'option1') {
                ...
                window.location.href='bullyingflashcard'; }
    ...
    if (supportButton) {
     ...
     // Event listerner for the true or false

    trueOption.addEventListener('click', function() {
        if (lastSelectedOption !== trueOption) {
            trueOption.style.backgroundColor = 'Red';
            falseOption.style.backgroundColor = 'White';
            lastSelectedOption = trueOption; // Update lastSelectedOption
        } else {
            trueOption.style.backgroundColor = 'White'; // Deselect if already selected
            lastSelectedOption = null; // Reset lastSelectedOption
        }
    });

    falseOption.addEventListener('click', function() {
        if (lastSelectedOption !== falseOption) {
            falseOption.style.backgroundColor = 'Red';
            trueOption.style.backgroundColor = 'White';
            lastSelectedOption = falseOption; // Update lastSelectedOption
        } else {
            falseOption.style.backgroundColor = 'White'; // Deselect if already selected
            lastSelectedOption = null; // Reset lastSelectedOption
        }
    });
